...
directorio_trabajo = "articulos_x_procesar"
origen = "elcolombiano"
...
for i in files_csv:
    ...
    ####### ID del evento
    filename  = os.path.basename(i).removeprefix('elcolombiano_').removesuffix('.csv')
    ...
    evento["país"] = "Colombia" if pais in ["Colombia", "Colombia"] else "Otro País"
    evento["ubicacion_noticia"] = f"{departamento}, {municipio}, {provincia}"
...
directorio = "C:/Users/Asus/TFM_CREDITOS/TFM2"


if not os.path.exists(directorio):
    os.makedirs(directorio)

with open(f"{directorio}/noticias_estandarizadas_Colombianooo.json", "w", encoding="utf-8") as archivo:
